...
def test_not_applicable_is_terminal() -> None:
    for target in FindingStatus:
        with pytest.raises(TransitionError):
            advance_finding(FindingStatus.NOT_APPLICABLE, target, INSPECTOR)
...
def test_unfinalize_requires_supervisor() -> None:
    with pytest.raises(TransitionError):
        unfinalize(INSPECTOR)
    assert unfinalize(Actor("admin", is_human=True, is_supervisor=True)) is ProcessState.COMPLETED
