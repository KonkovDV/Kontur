"""Минимальные PDF-фикстуры для pypdfium2.

Helvetica — только латиница. Кириллица — через встроенный TTF (Arial/DejaVu).
"""
...
def contest_slice_font() -> Path | None:
    """TTF с кириллицей для синтетических PDF. None — PDF-слой среза skip."""
...
def cyrillic_pdf(
    ...
    *,
    ...
    """Векторный PDF: каждая строка — отдельный текстовый объект (отдельный токен)."""
