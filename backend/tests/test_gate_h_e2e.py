...
def test_executable_count() -> None:
    """Гейт H: минимум 15 executable правил (цель: ≥20 к 22–23.09)."""
    executable = [
        c for c in _REGISTRY.all_codes() if _REGISTRY.get(c)["coverage"] == "executable"
    ]
    assert len(executable) >= 15, (
        f"Ждали ≥15 executable, получили {len(executable)}: {sorted(executable)}"
    )
...
_CASES: list[tuple[str, tuple[str, ...], str, str]] = [
    ...
    # PZ-014: эл. мощность (3 слова якорь)
    ("PZ-014", ("Расчетная", "электрическая", "мощность"), "1200,0", "1200,0"),
    # PZ-016: водопотребление (3 слова якорь)
