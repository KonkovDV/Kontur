...
def test_executable_count() -> None:
    """Гейт H: минимум 13 executable правил (цель: ≥20 к 22–23.09)."""
    executable = [
        c for c in _REGISTRY.all_codes() if _REGISTRY.get(c)["coverage"] == "executable"
    ]
    assert len(executable) >= 13, (
        f"Ждали ≥13 executable, получили {len(executable)}: {sorted(executable)}"
    )
...
_CASES: list[tuple[str, tuple[str, ...], str, str]] = [
    ...
    # PZ-011: однословный якорь
    ("PZ-011", ("Квартирография",), "180", "180"),
    # PZ-012: якорь с дефисом
