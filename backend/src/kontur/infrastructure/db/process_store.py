...
Финальный протокол материализуется атомарно с переходом процесса в FINALIZED.
"""
...
from kontur.application.protocol import assemble_protocol, protocol_id_for_process
...
class TransactionUnavailableError(RuntimeError):
    """Финализация отклонена, потому что атомарность нельзя гарантировать."""
...
def validate_snapshot(snapshot: ProcessSnapshot) -> None:
    ...
    if snapshot.process_state is ProcessState.FINALIZED:
        ...
        if snapshot.protocol_id != protocol_id_for_process(snapshot.process_id):
            raise ValueError("protocol_id не является каноническим для process_id")
...
INSERT INTO protocols (
    ...
    %(protocol_id)s, %(object_id)s, 1, %(matrix_version)s, %(dataset_version)s,
...
def _is_unique_violation(exc: Exception) -> bool:
    return getattr(exc, "sqlstate", None) == _UNIQUE_VIOLATION
...
class PostgresProcessStore:
    ...
    def _protocol_payload(self, snapshot: ProcessSnapshot) -> dict[str, object]:
        protocol_id = snapshot.protocol_id
    ...
    def _assert_same_protocol(
        self,
        snapshot: ProcessSnapshot,
        payload: dict[str, object],
        existing: object,
    ) -> None:
        row = tuple(existing)  # type: ignore[arg-type]
        expected = (
            snapshot.object_id,
            1,
        ...
        same_payload = _canonical_json(_payload_dict(row[7])) == _canonical_json(payload)
        if tuple(row[:7]) != expected or not same_payload:
            raise ProtocolConflictError(
                f"protocol {snapshot.protocol_id} уже содержит другое содержимое"
            )
    ...
    def _materialize_protocol(
        ...
        payload = self._protocol_payload(snapshot)
        params["payload"] = _canonical_json(payload)
        existing = self._connection.execute(SELECT_PROTOCOL_SQL, params).fetchone()  # type: ignore[attr-defined]
        if existing is not None:
            self._assert_same_protocol(snapshot, payload, existing)
            return
        try:
            with transaction():  # type: ignore[operator]
        ...
        if existing is None:
            raise ProtocolConflictError(
                f"protocol {snapshot.protocol_id} unique race без видимой строки"
            )
        self._assert_same_protocol(snapshot, payload, existing)

    def _replace_files(self, snapshot: ProcessSnapshot) -> None:
    ...
    def save(self, snapshot: ProcessSnapshot) -> None:
        ...
        if snapshot.process_state is not ProcessState.FINALIZED:
            ...
            self._replace_files(snapshot)
            return

        transaction = getattr(self._connection, "transaction", None)
        if not callable(transaction):
            raise TransactionUnavailableError("connection не предоставляет transaction()")
        with transaction():
            self._connection.execute(ENSURE_OBJECT_SQL, params)  # type: ignore[attr-defined]
            locked = self._connection.execute(LOCK_PROCESS_SQL, params).fetchone()  # type: ignore[attr-defined]
            if locked is None:
                raise ProtocolConflictError(
                    f"process {snapshot.process_id} отсутствует перед финализацией"
                )
            self._materialize_protocol(snapshot, params, transaction)
            self._connection.execute(UPSERT_PROCESS_SQL, params)  # type: ignore[attr-defined]
            self._replace_files(snapshot)
