...
Oracle RT-G: «At-least-once доставка даёт ровно одну находку
и одну версию протокола.»
...
if TYPE_CHECKING:
    # redis.asyncio.Redis — только для тайп-чекера, не хард зависимость
...
async def get_cached_passport(
    client: "AsyncRedis",  # type: ignore[type-arg]
    file_hash: str,
