...
Двойное чтение — два независимых прохода regex по одному окну: первый матч
(primary) и последний (secondary). Совпадают — second_read_agrees=True;
расходятся — ABSTAIN на уровне правила при dual_read_required=True.

Поддерживаемые операторы (совместимы с comparators.py):
...
def _window(tokens: Sequence[PageToken], anchor_end: int) -> list[PageToken]:
    ...
    rest = [
        t
        for t in tokens[anchor_end + 1 : anchor_end + 1 + _MAX_WINDOW_TOKENS]
...
def _regex_from_rule(rule: dict[str, object]) -> re.Pattern[str]:
    extractor = rule.get("extractor")
    if not isinstance(extractor, dict):
        raise TypeError("у правила нет extractor")
    pattern = extractor.get("regex")
    if not isinstance(pattern, str) or not pattern:
        raise ValueError(f"{rule.get('code')}: extractor.type=enum требует поле regex")
...
def _normalize(value: str, steps: tuple[str, ...]) -> str:
    ...
    "B 25" → "B25", "R 60" → "R60" — критично для сопоставления
    с упорядоченными списками вида ["B25", "R60"].
    """
    result = value
    for step in steps:
        if step == "nfc":
...
def extract_text(tokens: Sequence[PageToken], rule: dict[str, object]) -> TextHit | None:
    ...
    Совпадают → second_read_agrees=True; расходятся → False (ABSTAIN при
    dual_read_required=True на уровне evaluate_rule).
    """
    if not tokens:
        return None
