...
CI check, not a mock. Covers two failure modes:
...
RabbitMQ/Rin delivery is out of scope. Crash-before-commit is still a later
slice. This proves the PostgreSQL lock, immutable protocol and outbox
...
def _assert_identical_redelivery_is_idempotent(prefix: str) -> None:
    ...
    protocol_count = _scalar("SELECT count(*) FROM protocols WHERE id = %s", (protocol_id,))
    outbox_count = _scalar(
        "SELECT count(*) FROM integration_outbox WHERE protocol_id = %s", (protocol_id,)
    )
    stored_hash = _scalar("SELECT payload_sha256 FROM protocols WHERE id = %s", (protocol_id,))
