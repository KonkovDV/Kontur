...
| 1.5 | React + Node.js + Python, RabbitMQ | `web/`, `gateway/`, `backend/`, `docker-compose.yml`, `infrastructure/outbox.py` | `test_outbox_relay.py` | in_progress (outbox→брокер; HTTP не async pull) |
...
| 9.3 | Верификация, reason_code, атомарность, финализация, отмена | `application/review.py`, `domain/state_machines.py`, `POST /verify` `/complete` `/finalize` `/unfinalize`, триггер `protocols_finalized_is_immutable`, `materialize_finalized` | `test_review.py`, `test_state_machines.py`, `test_api.py`, `test_protocol_materialize.py`, `db/checks.sql` § 1–3, `scripts/check_protocol_concurrency.py` | in_progress (атомарный INSERT protocol+process+outbox; live PG retry/version/SKIP LOCKED; crash-before-commit нет; `split()` нет) |
...
| 9.6 | ИАИС «РиН»: только `PROTOCOL_FINALIZED`, УКЭП, 3 ретрая | `application/retry_policy.py`, `infrastructure/outbox.py`, ограничение `sync_only_after_finalize` | `test_retry_policy.py`, `test_outbox_relay.py`, `db/checks.sql` § 8 | in_progress (outbox→брокер 1/5/15 мин; УКЭП и sandbox РиН отсутствуют) |
| 10 | Таблицы БД | `infrastructure/db/schema.sql`, `infrastructure/db/process_store.py` | `test_schema_sql.py`, `test_process_store.py`, job `db` | in_progress (снимок процесса, `process_findings` / `process_files`, комплектность, `audit_log`, `payload_sha256`, `integration_outbox`; Alembic нет) |
