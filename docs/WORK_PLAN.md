...
Д1 фикстуры среза (PZ-001 / KR-055 / AR-041) закрыты тестом `test_d1_extractor_fixtures.py`. Перевод мм→м только при явном суффиксе; голый `1200` не считается миллиметрами. Двойное чтение `1200 мм` и `1,2 м` в одном окне соглашается после перевода. Площадь IOS4-078 (`500×300` мм) не масштабируется как длина. ZU-125 остаётся в мм. Векторный PDF: `1200 мм` vs `1,2 м` даёт `actual_value=1.2`; `899 мм` — кандидат. 83 правила не перекрашены.
