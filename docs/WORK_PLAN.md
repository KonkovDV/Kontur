...
| Новые источники `approval_status` | Не включены. Ждут письменный ответ на вопрос 21. См. ADR-0013 |
...
Окно в `web/src/EvidenceViewer.tsx`: три синхронные панели ПД / РД / ИД. На `main` этого ещё нет, пока ветка плана не влита. В каждой файл, шифр, редакция, статус утверждения, polygon из сохранённых координат. Пустая стадия — «нет фрагмента», не нарушение. Подтверждение не является кнопкой по умолчанию. Горячие клавиши C / R / Q. Чекбоксы только на массовое подтверждение `CANDIDATE`. Отклонение поштучно, с `reason_code`. `MISSING_EVIDENCE` подтвердить как нарушение нельзя.
...
Exit пятницы: загрузил учебный комплект, увидел три панели, подтвердил одну находку, отклонил одну с причиной, массово подтвердил отмеченные `CANDIDATE`, увидел, что сессия не закрывает Gate K. Это не заявление, что холодный Compose уже проходит весь сценарий раздела 9 брифа.

### Д3, 26.09

Вынос L1–L7 в существующий worker — только после окна Д2. Новый брокер не заводить. Метрики раздела 14 публиковать только с n, abstention и 95% CI на validation, разбитой по `object_id`. Своей frozen validation нет. Adversarial-пакет — черновик PR #115: не `Closes #80`, пока нет живого раннера и пока нет skew, OCR-расхождения, VLM и запрета класть текст страницы в system prompt. Тест на вложения PDF не писать (`GAP-EMB`).
