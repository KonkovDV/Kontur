...
`split()` не заменять заглушкой `evidence_group_id`. Демо раздела «три панели» его не требует. OCR-ветки `16f3a06` и `2ddc2b3` не мержить. `scan_tokens_for_injection` подключать к `_pages_from_blobs` отдельным PR, без заявления, что #84 закрыт: текст страницы по-прежнему недоверенные данные и не идёт в system prompt.
