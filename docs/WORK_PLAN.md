...
Календарь ниже повторяет дни брифа. Числа покрытия на снимке 23.09: **44** executable, **83** extractor_missing, **1** advisory, **4** source_missing. Живой снимок после нескольких явных единиц в одном пункте: **55** / **72** / **1** / **4**. `GH_SITUATION_2026_09_21.md` остаётся архивом 21.09. Гейты I, J, K и L открыты. Сводная таблица ответов организатора от 26.09.2026 записана в `ORGANIZER_ANSWERS_2026_09_26.md`. Канал отправки вопросов 1–22 из репозитория не зафиксирован.
...
Загрузка `POST /documents/upload` ставит прогон в поток процесса API (issue #177). Отдельный брокер и compose-сервис не добавлялись. Выбор эталона, CLI и учебный комплект считаются в вызывающем вызове и возвращают уже посчитанную запись.

Живой список держит полосу «Требует внимания» для `LOW_QUALITY`, `ABSTAIN`, `NOT_COMPARABLE`, `NOT_APPLICABLE` и `CLARIFICATION_REQUIRED`. На карточке отказа написано «Система не смогла проверить», и это не нарушение. `MISSING_EVIDENCE` остаётся в «Не проверялось». Подтверждение не является кнопкой по умолчанию.
...
### Что по-прежнему не делается

| Пункт | Почему стоп |
|---|---|
| `РАЗМЕЧЕННЫЙ_TEST__213` как frozen val | Таблица 26.09 скрытый тест не открывает. Карантин |
| Какая редакция ТЗ актуальна | Ответ 26.09: комплект 17.08.2026, ТЗ 1.1. Версия 11 августа — архив |
