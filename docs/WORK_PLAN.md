...
Карточка `PZ-001` по HTTP: фрагменты ПД и РД со страницей, `polygon_norm` в [0;1] и PNG листа. Стадия ИД в этой карточке отсутствует — панель «нет фрагмента», не нарушение. `POST select` на явный «не утв.» — 409, basis не становится `INSPECTOR_SELECT`.
