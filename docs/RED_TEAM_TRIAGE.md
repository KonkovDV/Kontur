...
|---|---|---|---|---|
| RT-2609-15 | S1 | Ручки API возвращают `NotImplementedError`; проверки приёма, роли и повторы не подключены к FastAPI и к шлюзу | Требует слоя репозиториев и решения по хранилищу | подключить `intake`, `rbac`, `retry_policy` в `presentation/api.py`; лимит тела запроса поставить на уровне ASGI и обратного прокси, а не на уровне обработчика |
...
| RT-2609-18 | S2 | React и Node не проверяются в CI; шлюз передаёт опцию `limit`, которой нет в `http-proxy-middleware` v3 | Нужны lock-файлы и решение по тулчейну | job `frontend` с `npm ci` и `tsc --noEmit`; ограничение размера тела перенести в `express`/обратный прокси |
...
| RT-2609-20 | S3 | `registry.DEFAULT_ROOT` вычисляется во время импорта модуля | Косметика, но модуль неимпортируем без каталога матрицы | ленивое разрешение пути |
