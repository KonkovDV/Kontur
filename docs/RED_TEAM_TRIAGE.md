...
|---|---|---|---|---|---|---|
| RT-2609-01 | RT-E | S1 | Код параметра приходит из PDF с неразрывным дефисом, NBSP, полноширинными цифрами или кириллической «Р» вместо латинской «P» | Код не совпадает ни с одним из 132 правил: находка исчезает без сообщения об ошибке | NFKC, свёртка дефисов и пробелов, консервативная свёртка омоглифов в `domain/rule_codes.py` | `test_rule_codes.py::test_typography_from_pdf_does_not_silently_break_the_code`, `::test_cyrillic_homoglyph_is_folded_only_when_result_is_latin` |
