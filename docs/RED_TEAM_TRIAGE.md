...
| ID | Набор | Класс | Атака | Цена отказа | Исправление | Регрессия |
|---|---|---|---|---|---|---|
| RT-2609-16 | RT-E | S1 | Подозрение без `evidence_group_id`, спам дублей, автомат пишет CONFIRMED | Ложный юридический статус или очередь из копий | `SuspicionSignal` только со статусом SUSPICION; дедупликация по группе; CONFIRMED отвергается | `test_suspicion.py` |

УКЭП и sandbox ИАИС «РиН» (п. 9.6) этим модулем не закрываются. Сигнал не
сверяет проект со СНиП (ADR-0006) и не подменяет `ABSTAIN` компаратора.
...
| ID | Набор | Класс | Атака | Цена отказа | Исправление | Регрессия |
|---|---|---|---|---|---|---|
| RT-2709-07 | RT-B | S1 | Белый/скрытый текст внутри CropBox читается как штамп | Ложный паспорт, ложная находка | СКО яркости растра vs текстовый слой; `text_render_agreement=False` обнуляет штамп | `test_visual_text.py`, `test_rt_suites.py::test_rt_b_hidden_text_layer_blocks_automatic_finding` |
| RT-2609-27 | RT-G | S2 | Процесс только в RAM | После рестарта теряется состояние выгрузки | `ProcessStore` + снимок; Postgres-адаптер по `schema.sql`; находки не персистятся | `test_process_store.py` |

`text_render_agreement=True` ставится только после выборки пикселей. Placeholder
`protocol_id` при финализации — для CHECK, не протокол ТЗ. ИАИС «РиН» не вызывается.
