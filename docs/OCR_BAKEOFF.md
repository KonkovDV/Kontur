...
Тот же день, образ `kontur-core` (Tesseract 5.5, веса eslav по `ocr_weights.lock.json`), код смонтирован поверх `/app/src`.
До этого дня eslav в образе не собирался: RapidOCR 3.9 отвергает строку `"onnxruntime"` и ждёт свой Enum, адаптер молча оставался на Tesseract.
Кроп строки подаётся в eslav без детектора (`use_det=False`). На выборке 600 строк детектор с распознаванием дал долю 0.64, распознавание по кропу — 0.695.
Второе чтение кропа значения в рантайме идёт так же. Пословные рамки дробили «1200» на «1» и «200», и в образе срез PZ-001 / AR-041 уходил в ложный `ABSTAIN`.

| Движок | n | mean_ca | tz_low | gate_i_low | Время |
|---|---|---|---|---|---|
| eslav PP-OCRv5 mobile | 5935 | 0.794 | 0.663 | 0.632 | 88 с, 1 поток |
| Tesseract 5.5 `rus+eng` | 5935 | 0.777 | 0.635 | 0.603 | 117 с, 6 потоков |

Разница с локальным Windows-замером (tz_low≈0.487) — в основном версия Tesseract и tessdata, не движок.
На выборке 593 строк перерисовка из исходных PDF в 300 dpi долю верных строк не подняла (0.54 → 0.52).
Часть промахов — дефекты эталона: перевёрнутый текст вертикальных граф, служебный невидимый слой, битая кодировка шрифта.
Пороги 0.95 и 0.97 не взяты. SILVER не GOLD. Гейт I открыт.
