...
| GAP-OCR-ROT | Поворот страницы | Полный лист с `/Rotate` читается с кадра pdfium. Кроп значения при `rotate` 90/180/270 берётся с того же видимого кадра, пустой прямой проход пробует 90/180/270. Гейт I открыт: SILVER не порог | гейт I |
| GAP-STAMP | Печать поверх текста | Красная и синяя печать выбеливаются до OCR. Толстое чёрное кольцо на кропе значения даёт `LOW_QUALITY`, coverage=0, не кандидат. Волосная рамка ячейки печатью не считается. Чёрное по чёрному без такого кольца не отделяется. SILVER 26.09: n=5935, исключено 0, gate_i_low≈0.438. Порог не взят. Гейт I открыт | гейт I |
...
| GAP-FREE-SEARCH | Свободный поиск как `SUSPICION`, не 133-й параметр Матрицы | Ответ 26.09, пункты 17 и 19: модуль High и только гипотезы вне Матрицы. Реестр `MATRIX_GAP` этот модуль не исполняет | конкурсный срез |
| GAP-DOCX-XML | DOCX и XML приняты, параметры не извлечены | Явный `ACCEPTED_UNPARSED`, не тихий пропуск и не `UNSUPPORTED_FORMAT` | после среза |
...
| GAP-CAP-OCR | OCR `MEASURED`, таблицы `UNAVAILABLE`, чертёж `DEGRADED` | SILVER 26.09 PDF_TEXT_LAYER: n=5935, stamp_excluded=0, coverage=1, gate_i_low≈0.438, порог 0.97 не взят. Ранее 23.09 n=5935, gate_i_low≈0.434; OBJ-VIOLATION-EXAMPLES n=3013, gate_i_low≈0.426. Не GOLD. Гейт I открыт | гейт I |
