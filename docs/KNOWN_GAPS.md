...
> **ТЗ v1.1** (авторитетная редакция с 18.09.2026; v1.0 от 11.09 — архивная).
> Ключевые уточнения из колла с организатором (18.09):
> - ИАИС «РиН» = только endpoint на нашей стороне (РиН сам забирает).
> - Авторизация не является критерием оценки хакатона.
> - Отсутствие документа ≠ нарушение; система показывает неполный комплект.
> - Массовая верификация через checkbox допустима; отклонение — поштучно.
> - Скрипт метрик организаторы вероятно опубликуют; формат frozen_val.jsonl совместим.
...
| GAP-IOS4-VAL | IOS4-078/079 executable на синтетике; recall на frozen val не измерен | экстрактор берёт первую сторону сечения, не площадь мм² | гейт J ← частично закрыт: тест `test_frozen_val_recall.py` добавлен |
| GAP-REDIS-FINDING | Redis SETNX для finding idempotency не встроен в пайплайн | PR #22 отклонён: drop на ошибке Redis нарушал stop-ship #11; fix в ветке `fix/redis-finding-safe-degrade` | гейт L |
| GAP-AUTH | Авторизация не реализована | По колл-записи: не является критерием оценки хакатона; оставить базовый Bearer | после 29.09 |
| GAP-IAAS-RIN | ИАИС «РиН» интеграция | Требуется только endpoint (GET /protocol/{id}); РиН забирает сам. Полная интеграция вне скоупа хакатона | гейт M |

Adversarial: RT-A…RT-I закрыты регрессией. Дубль находки в процессе закрыт
`put_finding` по `evidence_group_id`; cross-restart защита — GAP-REDIS-FINDING.
