...
| GAP-CAP-OCR | OCR и таблицы `UNAVAILABLE`; чертёж `DEGRADED` | Сетка PATH-штрихов читается. Сечение и конфигурация воздуховодов не измеряются. SILVER не GOLD. Гейт I открыт | гейт I |
