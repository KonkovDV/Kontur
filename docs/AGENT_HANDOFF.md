...
Скрытый тест не открывать. ПД без сведений об утверждении —
`CLARIFICATION_REQUIRED` (ADR-0015), пока инспектор не выберет файл.
Индекс и «В производство работ» не утверждение.
След таблицы в коде — раздел «След в репозитории» того же файла,
[`STACK_DEVIATIONS.md`](STACK_DEVIATIONS.md) и ADR-0016.
...
4b. Числовое семейство: разобраны все 57 правил с `extractor.type = number`,
   ...
   SPZU-025 и SPZU-026 исполняются площадью одного замкнутого прямоугольника
