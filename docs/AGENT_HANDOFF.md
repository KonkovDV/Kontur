...
1. Признак утверждения ПД без ослабления инварианта 5 (не «ГИП» и не пустая графа).
2. Живой k6 после пайплайна → строка в `PERFORMANCE.md` (не пустой `PARSING`).
3. 5 инспекторов → `USABILITY_RESULTS.md`.
