...
**Статус:** принято 15.09.2026, уточнено 16.09.2026. Круг ролей `unfinalize` расширен ADR-0015 (26.09.2026): инспектор, администратор или супервизор.
...
2. **Процесс:** `PENDING → PARSING → READY → VERIFYING → COMPLETED → FINALIZED`.
   ...
   `unfinalize(current, actor, reason)`: инспектор, администратор или супервизор, только из `FINALIZED`,
